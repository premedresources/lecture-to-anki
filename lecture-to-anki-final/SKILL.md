...
**Flag all diagram slides.** These cannot be fully captured by text cards. Note them in the output summary as candidates for Janus image occlusion processing. Do not attempt to recreate visual spatial relationships in text cards — text cards for diagram content should test function and label meaning, not spatial arrangement.
...
### File 3: `[CourseName]-[Topic]-janus.pdf`
...
- This file is dropped directly into Janus for image occlusion card generation — no other processing needed
- If no diagram slides were flagged, skip this file and note it in the summary

### Naming convention examples:
- `BioI-NervousSystem-cloze.txt` / `BioI-NervousSystem-qa.txt` / `BioI-NervousSystem-janus.pdf`
- `BioI-Immunology-cloze.txt` / `BioI-Immunology-qa.txt` / `BioI-Immunology-janus.pdf`
...
4. **Diagram slides flagged** — slide numbers or topics that should be sent to Janus for image occlusion processing
...
- [ ] Diagram slides have been extracted into a Janus PDF
...
- **Image occlusion card generation** — spatial label relationships on diagrams cannot be captured in text cards. Diagram slides are extracted into a filtered Janus PDF for you to upload to Janus directly.
