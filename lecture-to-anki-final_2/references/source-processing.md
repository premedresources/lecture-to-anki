...
2. One or more dedicated contrast Q&A cards — one per meaningful dimension of difference

**Critical rule:** A contrast card must name a specific dimension in the question and show how both concepts diverge on that dimension in the answer. "How does X differ from Y?" alone is too broad — it produces two stapled definitions, not a real contrast. Always frame as "How does X differ from Y in [specific dimension]?" The answer should end with a short discriminating summary where possible.

See card-patterns.md for the full contrast card rules, a worked bad example, and worked good examples.

**High-priority contrast pairs to always card:**
- Sympathetic vs. Parasympathetic (neurotransmitter used; effects on target organs; triggering conditions)
- Afferent vs. Efferent (direction of signal; sensory vs. motor)
- Somatic vs. Autonomic (conscious vs. involuntary; target tissue)
- CNS vs. PNS (what's included; structural role)
- Short-term vs. Long-term memory (storage location; mechanism of consolidation)
- Memorization vs. Skill learning (speed; cellular mechanism; durability)
- Primary vs. Secondary immune response (speed; mechanism; role of memory cells)

For each contrast card, the answer must name both sides and show how they diverge on the stated dimension — not just describe each one independently.
